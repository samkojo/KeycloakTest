<template>
  <div id="app">
    ...
    <div>
      <button class="btn" v-on:click="action('public')">Action for public</button>
      <button class="btn" v-on:click="action('secured')">Action for Users</button>
      <button class="btn" v-on:click="action('admin')">Action for Admin</button>
    </div>
    <div>
...
export default {
  ...
  methods: {
    action: function (role){
      axios
        .get("http://localhost:3000/service/"+role, {
          headers: {
            Authorization: "Bearer " + this.keycloak.token
          }
        })
